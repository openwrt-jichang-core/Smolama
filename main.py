...
def update_leaderboard_from_results(results):
    ...
    通过的记录参与排名(按总耗时升序)；失败的只额外记录，不参与排名。"""
    if not results:
        return
...
def _merge_leaderboard_entry(lb_map, host, model, tests):
    ...
    elapsed_total = round(sum(t.get("elapsed") or 0 for t in tests), 2)
    key = f"{host}|{model}"
    ...
    entry.update({
        ...
        "passed": passed,
        "total": total,
        "elapsed_total": elapsed_total if total else None,
...
def get_leaderboard(auth=Depends(require_auth)):
    lb = _load_json_file(LEADERBOARD_FILE, [])
    passed = sorted(
        [e for e in lb if e.get("status") == "pass"],
        key=lambda e: e.get("elapsed_total") if e.get("elapsed_total") is not None else float("inf"),
    )
    for i, e in enumerate(passed):
        e["rank"] = i + 1
    failed = [e for e in lb if e.get("status") != "pass"]
    return {"ranked": passed, "failed": failed}
...
def leaderboard_retest(body: RetestIn, auth=Depends(require_auth)):
    ...
    tests = [
        ...
        for name, status, detail, elapsed in raw_tests
    ]
